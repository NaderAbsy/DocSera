...
export default function DoctorsTable({ apiUrl }) {
  ...
  const fetchDoctors = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/getAllDoctores?${query}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      const json = await res.json();
      if (!json.success) throw new Error("Failed to fetch doctors");
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await fetch(`https://localhost:44316/api/Admin/deleteDoctore/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <table className="min-w-full border border-gray-200 bg-white shadow-md rounded-lg">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="px-4 py-2 border">Work Time</th>
        ...
        <tbody>
          {doctors.map((doc) => (
            <tr key={doc.id} className="text-center hover:bg-gray-50">
              ...
              <td className="px-4 py-2 border">8:00 AM to 4:00 PM</td>
              <td className="px-4 py-2 border">
                <button
